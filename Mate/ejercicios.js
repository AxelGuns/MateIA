...
async function obtenerEjercicioDeIA(tema, nivel) {
   ...
   function mostrarEjercicioActual() {
    ...
    // 1. Remover todos los event listeners existentes primero (opcional, para evitar duplicados)
    opcionesItems.forEach(item => {
        item.removeEventListener('click', manejarClickOpcion);
        item.classList.remove('seleccionada', 'correcta', 'incorrecta');
    });
    
    // 2. Agregar clase "bloqueada" y estilos
    opcionesItems.forEach(item => {
    ...
    // 4. Mostrar feedback
    mostrarFeedback(opcionIndex === ejercicio.respuestaCorrecta);
    
    // 5. Habilitar siguiente
...
function manejarClickOpcion(e) {
  ...
  function mostrarFeedback(esCorrecta) {
    ...
    if (esCorrecta) {
        ...
        // Marcar la opción seleccionada como correcta (mostrará palomita)
        opcionesItems[respuestasUsuario[ejercicioActual]].classList.add('correcta');
    } else {
        feedbackCorrecto.style.display = 'none';
        ...
        // Marcar la opción seleccionada como incorrecta (tachita)
        opcionesItems[respuestasUsuario[ejercicioActual]].classList.add('incorrecta');
        
        // Marcar la respuesta correcta con palomita
