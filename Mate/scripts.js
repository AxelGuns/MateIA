...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.puntosGlobales += points;

    const pointsDisplay = document.getElementById('points-display');
    if (pointsDisplay) {
        pointsDisplay.textContent = window.puntosGlobales;
    }

    localStorage.setItem('mateia_puntos_globales', window.puntosGlobales);

    checkAchievementsBasedOnPoints(window.puntosGlobales);
}
    ...
    window.puntosGlobales = parseInt(localStorage.getItem('mateia_puntos_globales')) || 0;

    // Actualiza los puntos globales y sincroniza en pantalla y almacenamiento
    function actualizarPuntosGlobales(delta) {
        window.puntosGlobales += delta;
        localStorage.setItem('mateia_puntos_globales', window.puntosGlobales);
        document.querySelectorAll('.puntos-globales').forEach(el => el.textContent = window.puntosGlobales);
    }
    ...
    // Cargar puntos y progreso guardado del usuario
function loadSavedUserData() {
    // ✅ Cargar puntos globales
    const puntosGuardados = parseInt(localStorage.getItem('mateia_puntos_globales')) || 0;
    window.puntosGlobales = puntosGuardados;
